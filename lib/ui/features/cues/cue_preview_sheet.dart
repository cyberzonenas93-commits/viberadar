...
Future<void> showCuePreviewSheet({
  ...
  ref.read(cueProvider.notifier).generateForTrack(
...
Future<void> showCueCrateSheet({
  ...
  ref.read(cueProvider.notifier).reset();
  ref.read(cueProvider.notifier).generateForCrate(
      tracks, cloudTracks: cloudTracks, useAi: useAi);
  await showModalBottomSheet<void>(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
...
class _CuePreviewSheetState extends ConsumerState<_CuePreviewSheet> {
  ...
  Future<void> _detectVdjRoot() async {
    ...
    if (mounted) setState(() { _vdjRoot = root; _vdjRootLoading = false; });
  }
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: AppTheme.edge, borderRadius: BorderRadius.circular(2)),
          ),
          // Header
          Padding(
            padding: const EdgeInsets.fromLTRB(20, 8, 20, 0),
            child: Row(children: [
              const Icon(Icons.piano_rounded, color: AppTheme.cyan, size: 20),
          ...
          Expanded(child: ListView(
            ...
            children: [
              // Progress / status
              if (cue.isAnalyzing) _AnalyzingBanner(done: cue.progress, total: cue.total),
              if (cue.status == CueGenStatus.error)
                _ErrorBanner(message: cue.errorMessage ?? 'Unknown error'),
              // Results
              if (cue.isDone) const _EstimateNote(),
              if (cue.isDone) ...[
                ...cue.results.asMap().entries.map((e) => _ResultCard(
                  result: e.value,
                  expanded: _expandedResultIdx == e.key,
                  onToggle: () => setState(() =>
                    _expandedResultIdx = _expandedResultIdx == e.key ? null : e.key),
                  onRemoveCue: (cueId) => ref.read(cueProvider.notifier)
                      .removeCue(e.value.trackId, cueId),
                  onRenameLabel: (cueId, label) => ref.read(cueProvider.notifier)
              ...
              // Actions
              if (cue.isDone) _ActionsRow(
                cue: cue,
                vdjRoot: _vdjRoot,
                vdjRootLoading: _vdjRootLoading,
                showVdjConfirm: _showVdjConfirm,
                onShowVdjConfirm: () => setState(() => _showVdjConfirm = true),
                onCancelVdjConfirm: () => setState(() => _showVdjConfirm = false),
              ...
              if (cue.vdjWriteStatus == VdjWriteStatus.error)
                _ErrorBanner(message: cue.vdjWriteError ?? 'VirtualDJ write failed'),
            ],
          )),
        ]),
      ),
  ...
  Future<void> _writeToVdj() async {
    ...
    if (cue.results.length == 1) {
      await ref.read(cueProvider.notifier)
...
class _AnalyzingBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppTheme.panelRaised, borderRadius: BorderRadius.circular(10),
...
class _ResultCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppTheme.panelRaised, borderRadius: BorderRadius.circular(12),
        border: Border.all(color: AppTheme.edge),
      ),
      child: Column(children: [
        // Track header
...
class _CueRow extends StatefulWidget {
  const _CueRow({required this.cue, required this.onRemove, required this.onRename});
  final HotCue cue;
...
class _ActionsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      // VDJ confirmation
      if (showVdjConfirm && vdjRoot != null) _VdjConfirmCard(
        vdjRoot: vdjRoot!,
      ...
      Container(
        ...
        child: Row(children: const [
          Icon(Icons.album_rounded, color: AppTheme.textSecondary, size: 16),
          SizedBox(width: 8),
          Expanded(child: Text(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Write cues to VirtualDJ?',
        ...
        Text('Root: $vdjRoot',
          style: const TextStyle(
            color: AppTheme.cyan, fontSize: 11, fontFamily: 'monospace'),
          overflow: TextOverflow.ellipsis,
        ),
        const SizedBox(height: 12),
        Row(children: [
          TextButton(onPressed: loading ? null : onCancel,
...
class _ConfidenceChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = confidence >= 0.7 ? Colors.greenAccent
...
class _CueCountBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text('$count cue${count != 1 ? "s" : ""}',
...
class _ActionBtn extends StatelessWidget {
  const _ActionBtn({
    ...
    required this.onTap, this.isComplete = false, this.loading = false,
  });
  final IconData icon;
  final String   label;
  final Color    color;
  ...
  Widget build(BuildContext context) {
    return ElevatedButton.icon(
      ...
      label: Text(label),
      style: ElevatedButton.styleFrom(
        backgroundColor: isComplete ? Colors.green.withValues(alpha: 0.2) : color,
