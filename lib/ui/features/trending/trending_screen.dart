...
class _TrendingScreenState extends ConsumerState<TrendingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final regions = [
      ...
      ...{
        for (final t in allTracks)
          if (t.leadRegion.isNotEmpty) t.leadRegion,
      }.toList()..sort(),
    ];
    ...
    if (_selectedRegion != 'All') {
      ...
      filtered = filtered
          .where((t) => (t.regionScores[_selectedRegion] ?? 0) > 0)
          .toList();
    }
    ...
    if (_selectedRegion != 'All') {
      filtered.sort(
        (a, b) => (b.regionScores[_selectedRegion] ?? 0).compareTo(
          a.regionScores[_selectedRegion] ?? 0,
        ),
      );
    } else {
              ...
              child: filtered.isEmpty
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (context, i) =>
                          _TrackCard(track: filtered[i], rank: i + 1, ref: ref),
                    ),
            ),
          ],
        ),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: AppTheme.glass(
            radius: 14,
            border: isTop3
                ? rankColor!.withValues(alpha: _hovered ? 0.55 : 0.34)
                : (_hovered
                      ? AppTheme.cyan.withValues(alpha: 0.32)
                      : AppTheme.hairline),
            glowShadow: _hovered
                ? AppTheme.glow(
                    isTop3 ? rankColor! : AppTheme.cyan,
                    blur: 20,
                    opacity: 0.10,
                  )
                : null,
