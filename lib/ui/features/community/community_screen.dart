    ...
    return Column(
      ...
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [_FeedTab(), _FeaturedTab(), _TrendingTab()],
          ),
        ),
      ],
    );
...
class _FeedTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return uploadsAsync.when(
      ...
      data: (all) {
        final uploads = all
            .where((u) => !blocked.contains(u.uploadedBy))
            .toList();
...
class _TrendingTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return uploadsAsync.when(
      ...
      data: (uploads) {
        final sorted =
            uploads.where((u) => !blocked.contains(u.uploadedBy)).toList()
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: AppTheme.glass(
            radius: 14,
            border: _hovered
                ? AppTheme.pink.withValues(alpha: 0.32)
                : AppTheme.hairline,
            glowShadow: _hovered
                ? AppTheme.glow(AppTheme.pink, blur: 20, opacity: 0.10)
                : null,
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  children: [
                    ...
                    if (t.genre.isNotEmpty)
                      Positioned(
                        top: 8,
                        left: 8,
                        child: _Badge(t.genre, AppTheme.violet),
                      ),
                    // BPM badge
                    if (t.bpm > 0)
                      Positioned(
                        top: 8,
                        right: 8,
                        child: _Badge('${t.bpm}', AppTheme.amber),
                      ),
                    // Play overlay
                    if (_hovered)
                      Positioned.fill(
                    ...
                    Positioned(
                      bottom: 4,
                      right: 4,
                      child: _ModerationMenuButton(
                        onSelected: (v) => _onModerationSelected(context, v),
                      ),
                    ),
                  ],
                ),
  ...
  void _onModerationSelected(BuildContext context, String value) {
    ...
    if (value == 'report') {
      showReportDialog(
        ...
        contentType: 'upload',
        contentId: t.id,
        reportedUid: t.uploadedBy,
      );
    } else if (value == 'block') {
      showBlockDialog(
        context,
        ref,
        blockedUid: t.uploadedBy,
        blockedName: t.uploaderName,
      );
    }
  }
...
class _EmptyFeed extends StatelessWidget {
  @override
  Widget build(BuildContext context) => const VibeEmptyState(
    icon: Icons.headphones_rounded,
    title: 'No uploads yet',
    subtitle: 'Be the first to share a track with the community!',
  );
}
