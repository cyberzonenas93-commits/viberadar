...
class AggregatedPlaylist {
  ...
  String get sourceLabel =>
      sources.map((s) => '${s[0].toUpperCase()}${s.substring(1)}').join(' + ');
}

/// Fetches real curated/editorial playlists from Spotify and Apple Music.
///
/// Spotify, Apple Music, and YouTube calls are routed through backend proxy
/// Cloud Functions — no API keys are held in the app bundle.
/// Deezer uses its public unauthenticated API (no key required).
class PlaylistAggregationService {
  PlaylistAggregationService({FirebaseFunctions? functions})
  ...
  /// [FirebaseFunctions.instance]. Lazy to avoid requiring Firebase init at
  /// construction time.
  FirebaseFunctions get _functions =>
      _injectedFunctions ?? FirebaseFunctions.instance;

  // ── Spotify proxy helper ───────────────────────────────────────────────────
  ...
  Future<List<AggregatedPlaylist>> _fetchSpotifyPlaylists({
    ...
    try {
      ...
      dev.log('Fetching Spotify featured playlists via proxy',
          name: 'PlaylistAggregation');
      final featuredData = await _spotifyGet(
        'browse/featured-playlists',
        {'limit': '10', 'country': 'US'},
      );

      final featItems = featuredData['playlists']?['items'] as List? ?? [];
      for (final item in featItems.take(5)) {
        ...
        if (playlistId != null) {
          final tracks =
              await _fetchSpotifyPlaylistTracks(playlistId, limit: limit);
      ...
      if (genre != 'All') {
        final catData = await _spotifyGet('search', {
          'q': genre,
          'type': 'playlist',
          'limit': '5',
          'market': 'US',
        });
        final catItems = catData['playlists']?['items'] as List? ?? [];
        for (final item in catItems.take(3)) {
          final playlistId = item['id'] as String?;
          final name = item['name'] as String? ?? 'Playlist';
          final desc = item['description'] as String? ?? '';
          final images = item['images'] as List? ?? [];
      ...
      if (playlists.isEmpty) {
        dev.log('No Spotify playlists found, falling back to track search',
            name: 'PlaylistAggregation');
        final searchQuery =
            genre == 'All' ? 'top hits 2026' : '$genre top hits';
        final searchData = await _spotifyGet('search', {
          'q': searchQuery,
          'type': 'track',
  ...
  Future<List<AggregatedPlaylist>> _fetchAppleMusicCharts({
    ...
    try {
      // Fetch top songs chart via proxy
      final chartData = await _apple.getCharts(limit: limit);
      final charts = chartData['results']?['songs'] as List? ?? [];

      for (final chart in charts) {
        final chartName = chart['name'] as String? ?? 'Top Songs';
        final chartDataItems = chart['data'] as List? ?? [];
        final tracks = <PlatformTrackResult>[];

        for (final item in chartDataItems) {
      ...
      dev.log('Apple Music playlist fetch error: $e',
          name: 'PlaylistAggregation');
    }
  ...
  /// Uses YouTube Data API (via proxy) if available, otherwise falls back to
  /// Spotify search for YouTube-labeled tracks.
  Future<List<AggregatedPlaylist>> _fetchYouTubeTrending({
    String genre = 'All',
    int limit = 100,
  }) async {
    try {
      // Try YouTube Data API first (via proxy)
      final query =
          genre == 'All' ? 'trending music 2026' : '$genre music trending';
      final videos =
          await _youtube.searchMusic(query, limit: limit.clamp(1, 50));
      ...
      for (final searchQuery in queries) {
        try {
          ...
          for (final item in items) {
            ...
            final ytSearch =
                'https://www.youtube.com/results?search_query=${Uri.encodeComponent('$artists $name official')}';
  ...
  /// Deezer's public chart API requires no authentication — called directly.
  Future<List<AggregatedPlaylist>> _fetchDeezerCharts({
    ...
    try {
      ...
      final items =
          (data['data'] as List?) ?? (data['tracks']?['data'] as List?) ?? [];
